...
class Persona:
    ...
    # Método getter para consultar los libros en propiedad
    def get_libros_propiedad(self) -> list:
        return self._libros_propiedad

    # Método getter para obtener el diario
    def get_diario(self) -> 'Libro':
        return self._diario

    # Método para hojear un libro (relación de uso)
    def hojear_libro(self, libro: 'Libro') -> None:
        print(f"{self.get_nombre()} está hojeando {libro}")
    ...
    def __str__(self) -> str:
        if self._hobbies:
            hobbies_str = ", ".join(self.get_hobbies())
        else:
            hobbies_str = "No tiene hobbies"

        if self._libros_leidos:
            libros_leidos_str: str = ", ".join(libro.get_titulo() for libro in self.get_libros_leidos())
        else:
            libros_leidos_str: str = "No ha leído libros"

        if self._libros_propiedad:
            libros_propiedad_str: str = ", ".join(libro.get_titulo() for libro in self.get_libros_propiedad())
        else:
            libros_propiedad_str: str = "No tiene libros en propiedad"

        return (f"Persona(nombre={self.get_nombre()}, edad={self.get_edad()}, altura={self.get_altura()}, "
                f"activo={self.get_activo()}, hobbies=[{hobbies_str}], "
                f"libros_leidos=[{libros_leidos_str}], libros_propiedad=[{libros_propiedad_str}], "
                f"diario={self._diario.get_titulo()})")

    # Método mágico __gt__ para comparar la edad de dos personas
    def __gt__(self, otra_persona: 'Persona') -> bool:
        return self.get_edad() > otra_persona.get_edad()
